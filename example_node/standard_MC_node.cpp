...
int main(int argc, char** argv) {
    ...
    float TAKE_OFF_HEIGHT, LAND_HEIGHT, GRAV, HOV_THRUST, THRUSTtoGRAV, SCALE_GRAV, YAW_SAT_VALUE, SAT_XY_VALUE, SAT_Z_VALUE, BIAS_Z_VALUE, HEARTBEAT_RATE;
    nh.getParam("/take_off_height", TAKE_OFF_HEIGHT);
    nh.getParam("/land_height", LAND_HEIGHT);
    nh.getParam("/gravity", GRAV);
    nh.getParam("/hover_thrust", HOV_THRUST);
    nh.getParam("/yaw_saturation", YAW_SAT_VALUE);
    nh.getParam("/xy_saturation", SAT_XY_VALUE);
    nh.getParam("/z_saturation", SAT_Z_VALUE);
    nh.getParam("/z_bias", BIAS_Z_VALUE);
    nh.getParam("/heartbeat_rate", HEARTBEAT_RATE);

    THRUSTtoGRAV =  HOV_THRUST/GRAV;
    SCALE_GRAV = 1.0/GRAV;
